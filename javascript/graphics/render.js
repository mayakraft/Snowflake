...
function drawSnowflakeOneArm(position){
	...
	var HEX_BRANCH = [ {x:1, y:0}, {x:.5,y:-0.866}, {x:-.5,y:-0.866}, {x:-1, y:0}, {x:-.5,y:0.866}, {x:.5,y:0.866} ];
	recursiveDrawLoop(this.tree, position, 0);
	function recursiveDrawLoop(node, position, angle){
		...
		if(node.right != undefined){
			var end = {x:(position.x + LENGTH * HEX_BRANCH[mod6(node.right.rBranches + angle)].x),
			           y:(position.y + LENGTH * HEX_BRANCH[mod6(node.right.rBranches + angle)].y)};
			line(position.x, position.y, end.x, end.y);
			recursiveDrawLoop(node.right, end, mod6(angle+1) );
			recursiveDrawLoop(node.right, end, mod6(angle-1) );
		}
		//first go to the bottom of tree, following the main stem
		if(node.left != undefined){
			var end = {x:(position.x + LENGTH * HEX_BRANCH[mod6(node.left.rBranches + angle)].x),
			           y:(position.y + LENGTH * HEX_BRANCH[mod6(node.left.rBranches + angle)].y)};
			line(position.x, position.y, end.x, end.y);
			recursiveDrawLoop(node.left, end, angle);
		}
	}
}
...
function drawSnowflake6Sides(position){
	...
	for(var angle = 0; angle < 6; angle += 2)
		recursiveDrawLoop(this.tree, position, angle);
	...
	function recursiveDrawLoop(node, position, angle){
		...
		var LENGTH = 100/Math.pow(node.generation+1, .9);
		if(node.left != undefined){
			var end = {x:(position.x + LENGTH * HEX_BRANCH[int(node.left.rBranches + angle)%6].x),
			           y:(position.y + LENGTH * HEX_BRANCH[int(node.left.rBranches + angle)%6].y)};
			line(position.x, position.y, end.x, end.y);
			recursiveDrawLoop(node.left, end, angle);
		}
		if(node.right != undefined){
			var end = {x:(position.x + LENGTH * HEX_BRANCH[int(node.right.rBranches + angle)%6].x),
			           y:(position.y + LENGTH * HEX_BRANCH[int(node.right.rBranches + angle)%6].y)};
			line(position.x, position.y, end.x, end.y);
			recursiveDrawLoop(node.right, end, angle);
		}		
	}
}
